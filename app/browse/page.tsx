    ...
    return (
        <div className="mx-[18%] p-[20px]">
            <div className="personal w-full flex border-pink-600">
                <div className="p-[10px] text-pink-600 font-bold border-pink-600">PERSONAL CAUSES</div>
            </div>
            <div className="filter-container flex w-full p-[10px] mb-[25px]">
                <div className="filter-categories w-full">
                    ...
                    <div className="flex gap-2 cursor-pointer h-[40px]">
                        {categoriesOptions.map((cat) => (
                            <button
                                key={cat.key}
                                className={`px-3 py-1 rounded-[24px] transition-colors duration-150 border cursor-pointer ${filterObject.category === cat.key ? 'bg-pink-600 text-white' : 'bg-white text-gray-700'}`}
                                onClick={() => setFilterObject({ ...filterObject, category: cat.key })}>
                                {cat.label}
                            </button>
                        ))}
                    </div>
                ...
                <div className="filter-dropdown flex gap-2 w-full">
                    <Dropdown
                        ...
                        onSelect={(value) => setFilterObject(value)}
                    />
                    <Dropdown
                        label="SORT BY"
